...
- **Flyway:** скрипты в `db/migration/` — `V1` (схема), `V2` (демо‑данные: несколько постов и комментариев для пустой БД). Состояние версий в `flyway_schema_history`; уже применённые миграции не выполняются повторно.
